...
__src/App.jsx__. `App` is rendering the `PupForm` and `PupImage` components. In
the `PupImage` component, the `speedy` pup image is rendered and is not set to
...
**Note:** Because __PupContext.jsx__ now exports `PupContext` and the
...
(__src/index.jsx__) and wrap `App` with it.
...
Inside the x__src/context/PupContext.jsx__ file, create and export a function
